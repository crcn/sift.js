...
var GroupOperation = /** @class */ (function(_super) {
  ...
  function GroupOperation(params, owneryQuery, options, _children) {
    ...
    _this._childrenNext = function(item, key, owner) {
      var done = true;
      var success = true;
      if (_this._currentChildIndex < _this._children.length) {
        var currentChild = _this._children[_this._currentChildIndex];
        var result = currentChild.next(item, key, owner);
        if (result) {
          return result.then(_this._onChildDone);
        } else {
          _this._onChildDone();
        }
      }
  ...
  GroupOperation.prototype.childrenNext = function(item, key, owner) {
    var done = true;
    var success = true;
    var result = this._childrenNext(item, key, owner);
    this.done = done;
...
var EqualsOperation = /** @class */ (function(_super) {
  ...
  EqualsOperation.prototype.next = function(item, key, parent) {
    var _this = this;
    var result = this._test(item, key, parent);
    if (result) {
