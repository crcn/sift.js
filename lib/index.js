...
var BUILTIN_OPERATION_FILTERS = {
  $in: function $in(params, item) {
    return false;
  },
  $eq: function $eq(params, item) {
    return equals(item, params);
  },
  $and: function $and(params, item) {
...
var getValue = function getValue(root, propertyPath) {
  var current = root;
...
var createOperationFilter = function createOperationFilter(
  operation,
  params,
  options
) {
...
var createNestedFilter = function createNestedFilter(
  propertyPath,
  query,
  options
) {
...
var createQueryFilter = function createQueryFilter(query, options) {
  var filters = [];

  for (var property in query) {
    var params = query[property]; // operation
...
var createFilter = function createFilter(query, options) {
  if (options === void 0) {
    options = {};
  }
